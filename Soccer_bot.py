...
from Soccer_bot_const import *


headers = {"X-Auth-Token": api_token, "X-Response-Control": "minified"}


# Universal Time (UTC) is used as the official world reference for time. (-4 for eastern)
...
def get_table(connection, league_id):
    try:
        connection.request("GET", "/v1/competitions/{}/leagueTable".format(league_id), None, headers) #URI
    except:
        error(bot)
        return
...
def start(bot, update):
    string = "`/help ` command list\n`/table [league-code]` shows table for selected league\n`/fixtures [league-code] [matchday] [UTC]` shows fixtures for selected league and matchday, e.g. `/fixtures SA 13 2`\n`/live [league-code] [UTC] ` shows live fixtures for selected league\n`/remaining [league-code] [UTC]` shows remaining fixtures for selected league\n`/team [team-name] [days] [UTC]` shows fixture for team in the following days (all competitions)\n\nLeague codes:\n`BSA`  Brazilian Serie A\n`PL `  Premier League\n`ELC`  Championship\n`EL1`  League One\n`EL2`  League Two\n`DED`  Eredivisie\n`FL1`  Ligue 1\n`FL2`  Ligue 2\n`BL1`  Bundesliga\n`BL2`  2. Bundesliga\n`PD `  La Liga\n`SA `  Serie A\n`PPL`  Primeira Liga\n`DFB`  DFB Pokal\n`SB `  Serie B\n' 'UTC' = the basis for civil time today \n e.g. eastern time = -4"
    bot.send_message(chat_id=update.message.chat_id,
                     text=string, parse_mode="markdown")
    return
